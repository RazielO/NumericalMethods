<script lang="ts">
	...
	import Info from '$lib/algorithms/equationSolving/jacobi/Description.md';

	// Algorithm imports
	import { Jacobi } from '$lib/algorithms/equationSolving/jacobi/algorithm';
	import type { JacobiStep } from '$lib/algorithms/equationSolving/jacobi/type';
	...
	let title = 'Jacobi Method';
	const headers: string[] = ['n', 'solutions', 'errors'];
	let values: string[][];

	// Algorithm related variables
	let gauss: Jacobi;
	let steps: Array<JacobiStep>;
	let results: string[];
	let matrix: number[][];
	let size: number = 3;
	...
	function solve() {
		gauss = new Jacobi(matrix, tolerance);
		steps = gauss.steps();
